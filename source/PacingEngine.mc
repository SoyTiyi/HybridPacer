...
// Stateless: reads mRestMs and mRaceCycle from the singleton via getApp().
// Single instance created in HybridPacerApp.initialize() as mPacing.
//
...
// Fixed HYBRID running distance: 8 km total (1 km per cycle × 8 cycles).
const RACE_TOTAL_KM as Float = 8.0f;
...
class PacingEngine {
    ...
    //   - TRANSITION_OUT → RUN: new km, with projected rest penalty.
    //
    // Parameters:
    //   targetTimeMs        — overall goal race time (ms).
    //   elapsedTotalMs      — committed time so far = mWorkMs + mRestMs (ms).
    //   distanceCompletedKm — kilometers already run = mRaceCycle.
    //
    ...
    function computeDynamicPaceTarget(targetTimeMs as Number, elapsedTotalMs as Number, distanceCompletedKm as Number) as Float {
        ...
        var distanceRemainingKm = RACE_TOTAL_KM - distanceCompletedKm.toFloat();
        ...
        var app        = getApp();
        var cyclesDone = app.mRaceCycle;  // Completed cycles (= km already run)
        var avgRestMs  = 0;
        if (cyclesDone > 0) {
            avgRestMs = app.mRestMs / cyclesDone;  // Integer division: mean rest ms per cycle
        }
        var cyclesRemaining = RACE_TOTAL_CYCLES - cyclesDone;
