...
- Named HYROX station display (SkiErg, Sled Push, Sled Pull, …)
...
### Added
- **Pause / resume** via the START/STOP button during any race state (RUN, ROXZONE, STATION).
...
- `HyroxPacerApp.initialize()` loads the persisted value with type-check, clamp to [40, 180], and fallback to 90 minutes.
...
### Added
- `HyroxPacerView.mc` — fully imperative rendering via `Dc` primitives (no `layout.xml`).
- **Three-band layout**: header (state + cycle counter), center (primary metric), footer (secondary / button hint).
- **Per-state screens**: WARMUP (goal time + GPS status), RUN (pace), ROXZONE (transition timer in yellow), STATION (station timer + active athlete), FINISH (total time + W/R ratio).
...
### Fixed
- Button routing bug: `HyroxPacerDelegate` rewritten using `BehaviorDelegate` callbacks.
...
- `mDynamicPaceTargetSec` member in `HyroxPacerApp` updated by `FSMController` on each WARMUP→RUN and ROXZONE_OUT→RUN transition.
- `HYROX_TOTAL_KM = 8.0f` constant.
...
### Added
- `HyroxFitSession.mc` — owns 7 `FitContributor.Field` handles.
...
- FIT fields defined: `hyrox_cycle_id`, `hyrox_fsm_state`, `roxzone_total_time`, `station_elapsed`, `active_athlete`, `pace_delta_deviation`, `work_rest_ratio`.
- `FIT_ID_*` constants to keep field IDs in sync with `fitcontributions.xml`.
- `mIsInitialized` fast-path guard in `HyroxFitSession` (no-op before `startRecording` or after `stopRecording`).
...
- Duration accounting: elapsed time (minus paused time) accumulated into `mWorkMs`, `mRestMs`, `mRoxzoneTotalMs` on each transition.
- `markLap()` → `GpsSessionManager.addLap()` on RUN→ROXZONE_IN and ROXZONE_OUT→RUN boundaries.
- **Doubles / relay mode**: `mActiveAthlete` flag, `accrueAthleteTime()` tracks per-athlete time.
- `STATE_WARMUP`, `STATE_RUN`, `STATE_ROXZONE_IN`, `STATE_STATION`, `STATE_ROXZONE_OUT`, `STATE_FINISH` constants.
- `HYROX_TOTAL_CYCLES = 8` constant.
