...
### Phase 1 — Foundation
App scaffold, `HybridPacerApp` singleton, `getApp()` global accessor, GPS + FIT session lifecycle stubs.

### Phase 2 — State Machine & Doubles Mode
Full 6-state FSM (`WARMUP → RUN → TRANSITION_IN → STATION → TRANSITION_OUT → FINISH`), 5-second debounce between transitions, doubles/relay mode with per-athlete time tracking.
...
### Device Expansion
- **Priority: High** — Add more popular hybrid-race devices: Fenix 7 series, Epix (Gen 2), Forerunner 255, Forerunner 745, Venu 3.
